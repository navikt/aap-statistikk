...
class InstitusjonsoppholdRepositoryImplTest {
    ...
    fun `lagre og hent igjen`(@Postgres dataSource: DataSource) {
        ...
        dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).lagre(behandlingId, perioder)
        }

        val res = dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).hent(behandlingId)
        }
    ...
    fun `skal overskrive eksisterende perioder ved ny lagring`(@Postgres dataSource: DataSource) {
        ...
        dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).lagre(behandlingId, initialPerioder)
        }
        ...
        dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).lagre(behandlingId, oppdatertePerioder)
        }
    ...
    fun `tom liste sletter eksisterende data`(@Postgres dataSource: DataSource) {
        ...
        dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).lagre(behandlingId, emptyList())
        }
    ...
    fun `hent returnerer null for ikke-eksisterende behandling`(@Postgres dataSource: DataSource) {
        ...
        val res = dataSource.transaction {
            InstitusjonsoppholdRepositoryImpl(it).hent(ikkeEksisterendeBehandlingId)
        }
