...
class TilkjentYtelseTabell : BQTable<TilkjentYtelse> {
    ...
    override fun parseRow(fieldValueList: FieldValueList): TilkjentYtelse {
        ...
        val behandlingsReferanse = fieldValueList.get(FeltNavn.BEHANDLINGSREFERANSE.feltNavn).stringValue

        val tilkjentYtelsePerioder = fieldValueList.get(FeltNavn.PERIODER.feltNavn).repeatedValue.map {
            ...
            mapOf(
                ...
                FeltNavn.PERIODER.feltNavn to value.perioder.map {
                    mapOf(
                        ...
                        FeltNavn.GRADERING.feltNavn to it.gradering.toDouble(),
                        FeltNavn.DAGSATS.feltNavn to it.dagsats.toDouble(),
                    )
                })
        )
    }
